...
namespace Screen_Capture {
    ...
    DUPL_RETURN X11FrameProcessor::Init(std::shared_ptr<Thread_Data> data, const Window &selectedwindow)
    {
        ...
        XImage_ = XShmCreateImage(SelectedDisplay, DefaultVisual(SelectedDisplay, scr), DefaultDepth(SelectedDisplay, scr), ZPixmap, NULL,
                                  ShmInfo.get(), selectedwindow.Size.x, selectedwindow.Size.y);
    ...
    DUPL_RETURN X11FrameProcessor::ProcessFrame(const Monitor &curentmonitorinfo)
    {
        ...
        if (!XShmGetImage(SelectedDisplay, RootWindow(SelectedDisplay, DefaultScreen(SelectedDisplay)), XImage_, OffsetX(SelectedMonitor),
                          OffsetY(SelectedMonitor), AllPlanes)) {
    ...
    DUPL_RETURN X11FrameProcessor::ProcessFrame(Window &selectedwindow)
    {
        ...
        if (!XShmGetImage(SelectedDisplay, selectedwindow.Handle, XImage_, 0, 0, AllPlanes)) {
            return DUPL_RETURN_ERROR_EXPECTED;
        }
